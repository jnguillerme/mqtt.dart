...
class MqttMessagePublish extends MqttMessage {
  ...
  MqttMessagePublish.setOptions(String topic, String payload, [num msgID = 1, int QoS = 0, int retain = 0]) : 
                                                ...
                                                super.setOptions(PUBLISH, (QoS > 0) ? (6 + encodeUtf8(topic).length + encodeUtf8(payload).length) 
                                                                                    : (4 + encodeUtf8(topic).length + encodeUtf8(payload).length), 
                                                                         QoS, retain);
  ...
  bool operator == (MqttMessagePublish other) {
        return ( super == (other as MqttMessage)
  ...
  encodePayload() {
    // payload
    _buf.addAll(encodeUtf8(_payload));
  }
  ...
  num decodeVariableHeader(List<int> data)   {
    ...
    _topic = decodeUtf8(data, pos, topicLength);
    pos += topicLength;
